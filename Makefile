...
.PHONY: help check check-corpus check-inventory docs-gc docs-gc-apply audit-gt gt-gate status-preflight scorecard race align-ablate deploy deploy-storage deploy-worker \
...
help:
	...
	@echo "  make status-preflight — require BB fixture stamps before status regen"
...
status-preflight:
	venvs/audio/bin/python -m labeling.gt_release_gate --check-status-regen
