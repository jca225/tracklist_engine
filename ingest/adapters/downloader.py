...
def download_one(
    ...
    out_template = str(cfg.out_dir / f"{track_id}__{source.platform}__{source.player_id}.%(ext)s")
    ...
    if not final_path.exists():
        candidates = list(cfg.out_dir.glob(f"{track_id}__{source.platform}__{source.player_id}.*"))
        if not candidates:
            return Err(DownloadError(
                kind="disk", url=source.url,
...
def download_set_mix(
    ...
    if not final_path.exists():
        ...
        if not candidates:
            return Err(DownloadError(kind="disk", url=source_url,
                                     detail=f"expected output missing at {final_path}"))
        final_path = candidates[0]

    return Ok(SetAudioAsset(
        set_audio_id=None,
        set_id=set_id,
        platform=platform,
