...
from alignment.agentic.actions import REGISTRY, bind, plan_for
from alignment.agentic.belief import Observation, SpanBelief
from alignment.agentic.events import EventLog, replay_beliefs
...
from alignment.drivers.agentic import (
    pseudo_acceptance,
...
def test_live_context_from_set_accepts_explicit_fiber_gate(monkeypatch):
    ...
    from alignment import fp_placement_refine, mert_store
...
def test_ref_fp_for_span_prefers_claimed_stem_then_regular(monkeypatch):
    ...
    from alignment.agentic import live_runners as lr
    ...
    monkeypatch.setattr("alignment.fp_index.load", fake_load)
    # Also patch the late import path used inside ref_fp_for_span
    import alignment.fp_index as fp_index
