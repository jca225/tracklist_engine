...
def test_gain_curve_round_trips(tmp_path):
    """The fader ride survives save/load — the ground truth for audibility."""
    t = GroundTruthTrack(
        label="Galantis - You",
        track_id="x",
        ...
        set_end_s=2660.8,
        ref_start_s=0.2,
        gain_curve=((2527.8, 0.0), (2592.5, 0.9), (2660.8, 0.4)),
    )
    path = tmp_path / "gt.yaml"
    path.write_text(dump(GroundTruthSet(set_id="s", tracks=(t,))))
    match load(path):
        case Ok(gt):
            assert gt.tracks[0].gain_curve == (
                (2527.8, 0.0),
                (2592.5, 0.9),
                (2660.8, 0.4),
            )
...
def test_flat_unity_gain_curve_omitted(tmp_path):
    """A bare unity curve isn't serialized — absence already means 'audible'."""
    t = GroundTruthTrack(
        label="A - B",
        track_id="x",
        claimed_stem="regular",
        ...
        set_end_s=10.0,
        ref_start_s=0.0,
        gain_curve=((0.0, 1.0), (10.0, 1.0)),
    )
    text = dump(GroundTruthSet(set_id="s", tracks=(t,)))
    assert "gain_curve" not in text
...
def test_unalignable_placeholder_detection():
    ...
    assert (
        _placeholder_note("/s/stems/072__X/instrumental.flac", "g") is None
    )  # real demucs stem
