...
| P2 Mode A CLI | `python -m workspaces.alignment_prototype.daw_env --mode a` → shadow timeline; scorecard operator recipe in README; EXPERIMENTS ledger **SHADOW** pending BB audio run |
...
```bash
venvs/audio/bin/python -m workspaces.alignment_prototype.daw_env --set-id 1fsnxchk --mode a --set-dir ~/aligning/<bb12>
