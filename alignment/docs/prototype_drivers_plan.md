...
Status: **BUILT 2026-07-09** — `alignment/drivers/` + `make race`.
...
| Item | Path |
|---|---|
| Prototype root | `alignment/` |
| Driver contract (extend here) | `harness/contract.py`, `harness/driver.py`, `harness/merge.py`, `harness/axes.py` |
| Prediction record | `alignment/records.py` (`SpanPrediction`) |
| GT loader | `alignment/dataset.py` (`load_set`) |
