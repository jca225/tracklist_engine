...
Module home: `alignment/looptrace/` — inside the existing
...
- **`alignment/path_decode.py`** — the legacy segment decoder this
...
- Feature SR=22050, HOP=512 → FPS≈43.066 (path_decode); landmark fp uses NFFT=2048,
  ...
  HuBERT/chroma features cached at `alignment/.feat_cache/`
  (666 files present).
...
- `looptrace/` lives under `alignment/`, not top-level (repo
