...
CLI (oracle placement, mirrors the frozen path_decode --eval population):
    venvs/audio/bin/python -m alignment.looptrace.run \
...
from alignment.looptrace import (  # noqa: E402
    landmarks,
    loops,
    segments,
    selfsim,
    ...
    *,
    ...
    if hybrid:
        if ref_mel is None and ref_path is not None:
            from alignment.looptrace.residual import mel_cached
    ...
    if discrim and segs and song_pairs and ref_path:
        from alignment.looptrace.discrim import (
            reselect_instances,
        )
...
def _slope_band(row: dict, mix_series, ref_series, t) -> list[float]:
    ...
    from alignment.path_decode import _stretch_band
...
def main(argv: list[str] | None = None) -> int:
    ...
    from alignment.looptrace.audit import eval_population
