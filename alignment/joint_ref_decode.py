...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.decoder == "looptrace":
        from alignment.looptrace import (
            landmarks as lt_landmarks,
        )
        from alignment.looptrace.run import decode_span, mix_slice
        ...
        for idx, s in enumerate(spans):
            ...
            from alignment.looptrace.config import SEG_V1
    ...
    def _fiber_soft(ref_path: str):
        if ref_path not in fiber_soft_cache:
            from alignment.fibers import compute_fibers_soft
