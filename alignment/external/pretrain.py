...
Synthetic generator: alignment.synthetic_mix.generate

Examples:
  # Parse-only / pipeline smoke (no audio):
  venvs/audio/bin/python -m alignment.external.pretrain --dry-run \\
...
from alignment.external.feature_series import build_mix_bundle
...
def collect_unmix_examples(
    ...
    *,
    ...
    from alignment.mert_features import MertSpanExample

    if synthetic:
        from alignment.synthetic_mix.corpus import (
            slot_pools_for_mix as synthetic_slot_pools,
            targets_for_mix,
        )
        from alignment.external.unmixdb import _recording_id
