...
from labeling.als_io import load_als_xml, write_tempo_envelope  # noqa: E402

DEFAULT_TEMPLATE = Path.home() / "Desktop/big bootie 12 labeling Project/big bootie 12 labeling_fast.als"
...
def measure_times(path: Path) -> list[float]:
    ...
    xs = d if isinstance(d, list) else (
        d.get("measure_times") or d.get("beats") or next(iter(d.values())))
...
def grid_bpm(times: list[float], beats_per_measure: int) -> tuple[list[float], list[float]]:
...
def gt_boundaries(gt_path: Path) -> list[float]:
    ...
    bs = {round(float(r["set_start_s"]), 3) for r in rows
...
def track_segments(gt_path: Path, slot_bpm: dict[str, float], times: list[float],
                   *, beats_per_measure: int, round_bpm: float
    ...
    import numpy as np
    import yaml
    rows = [r for r in yaml.safe_load(gt_path.read_text())["tracks"]
            if r.get("track_id") and r.get("set_start_s") is not None]
    bed = sorted((r for r in rows if (r.get("claimed_stem") or "regular") != "acappella"),
                 key=lambda r: float(r["set_start_s"]))
    ts, bp = grid_bpm(times, beats_per_measure)
    ts_a, bp_a = np.array(ts), np.array(bp)
    natives = [slot_bpm[str(r.get("slot_label"))] for r in bed
...
def tempo_segments(times: list[float], *, beats_per_measure: int, round_bpm: float,
                   boundaries: list[float] | None) -> list[tuple[float, float, float]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--measures", type=Path,
    ...
    p.add_argument("--round-bpm", type=float, default=1.0,
    ...
    p.add_argument("--out", type=Path, default=Path.home() / "Desktop/bb12_tempo_test.als")
    ...
    if args.no_gt:
        segs = tempo_segments(times, beats_per_measure=args.beats_per_measure,
                              round_bpm=args.round_bpm, boundaries=None)
        src = "auto-grid"
    else:
        aligning = sorted(ALIGNING_ROOT.glob(f"{args.set_id}__*"))
        slot_bpm = slot_bpm_map(aligning[0] / "tracks") if aligning else {}
        segs = track_segments(args.gt, slot_bpm, times,
    ...
    args.out.write_bytes(gzip.compress(etree.tostring(root, xml_declaration=True,
                                                       encoding="UTF-8")))
    bpms = [v for _, _, v in segs]
    print(f"wrote {len(segs)} tempo segments ({n} points, step-shaped) -> {args.out}")
    print(f"  source: {src}; bpm range {min(bpms):.0f}-{max(bpms):.0f}, "
