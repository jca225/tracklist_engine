...
decode() {  # $1 set_id   $2 infer_pid (may be empty = already done)
  ...
  $PY -m workspaces.alignment_prototype.joint_ref_decode \
      --set-id "$sid" --decoder looptrace --workers 8 \
      --out "workspaces/alignment_prototype/out/${sid}_predicted_timeline_lt_v2.json" \
...
for sid in 1fsnxchk 2nvzlh2k; do
  echo "=================== $sid ==================="
  $PY -m workspaces.alignment_prototype.score_timeline_vs_gt \
      --set-id "$sid" --fibers \
      --timeline "workspaces/alignment_prototype/out/${sid}_predicted_timeline_lt_v2.json" \
      2>&1 | tail -45
done
