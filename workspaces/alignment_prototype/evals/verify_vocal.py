...
from workspaces.alignment_prototype.stem_placement import (  # noqa: E402
    FPS,
    hubert_of,
    place_joint,
)
...
def _wordset(path: Path | str | None) -> set[str]:
    ...
    if path is None:
        return set()
    from workspaces.alignment_prototype.lyrics_align import _norm, transcribe_words
...
def run_localize(set_dir: Path, win_s: float) -> None:
    ...
    cache = (
        _REPO
        / "workspaces/alignment_prototype/.cache/verify_windows"
...
def run_banded(set_dir: Path, set_id: str, win_s: float) -> None:
    ...
    if not mixv.is_file():
        sys.exit(f"no {mixv} — run the mix-vocal separation first")
    tl = _REPO / "workspaces/alignment_prototype/out" / f"{set_id}_fused_timeline.json"
...
def main() -> None:
    ...
    tmp = _REPO / "workspaces/alignment_prototype/.cache/verify_windows" / args.set_id
