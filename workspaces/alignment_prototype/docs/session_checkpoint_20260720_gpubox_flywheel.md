...
| Piece | Path |
|---|---|
| Aligning path remap (Mac abs → set_dir) | `workspaces/alignment_prototype/aligning_paths.py` + `_vocal_ref_path(..., set_dir=)` in `infer.py` / `live_runners.py` |
| Tests | `workspaces/alignment_prototype/tests/test_aligning_paths.py` (6 green) |
...
Allowlist must include: `core`, `analysis`, `labeling`, `eda/{__init__,alignment/__init__,mert_vectors}`, **`workspaces/section_hsmm`**, `workspaces/alignment_prototype`. Never full-repo rsync (`.claude` ~40G).
...
**Artifacts:** `workspaces/alignment_prototype/out/vast_agentic/`
