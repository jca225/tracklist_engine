...
Usage
-----
    venvs/audio/bin/python -m pws_aligner.capture_votes \\
...
Output: ``alignment/out/<set_id>_probe_votes.json``
...
3. Per-probe fingerprint cache at
   ``alignment/.cache/fp_index/<recording_id>__<stem>.landmark``
...
4. The predicted timeline at
   ``alignment/out/<set_id>_predicted_timeline.json``
...
def _build_probes(probe_names: tuple[str, ...]) -> dict[str, object]:
    ...
    from alignment.harness.contract import Probe  # noqa: F401
    ...
    for name in probe_names:
        ...
        try:
            if name == "fp":
                ...
                from alignment.harness.chroma_probe import (
                    ChromaProbe,
                )

                instances[name] = ChromaProbe()
            elif name == "hubert":
                from alignment.harness.hubert_probe import (
                    HubertProbe,
                )
...
def capture_votes(
    ...
    *,
    ...
    timeline_path:
        ...
        Defaults to ``alignment/out/<set_id>_predicted_timeline.json``.
    out_path:
        Path to write the output ``<set_id>_probe_votes.json``.
        Defaults to ``alignment/out/<set_id>_probe_votes.json``.
    probe_names:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "(default: alignment/out/<set_id>_probe_votes.json)"
        ),
    )
    p.add_argument(
        "--probes",
