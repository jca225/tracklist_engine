...
The scripts that produced each finding live in this folder. Run them from
the repo root — they assume `data/analysis/` and `data/db/` paths relative
to the project root, not to this file.
...
Reproduction: [bb_era_orthogonality.py](bb_era_orthogonality.py).
...
Reproduction: [bb_popularity.py](bb_popularity.py).
...
Reproduction: [bb_set_views_analysis.py](bb_set_views_analysis.py).
...
Reproduction: [bb_weekly_chart_analysis.py](bb_weekly_chart_analysis.py).
Source data: [aux_db_sync.py](aux_db_sync.py) ingests
...
Reproduction: [bb_spotify_charts.py](bb_spotify_charts.py)
(scrapes kworb.net per-track) →
[aux_db_sync.py](aux_db_sync.py) (ingests into
`aux.track_spotify_charts`) →
[bb_spotify_chart_analysis.py](bb_spotify_chart_analysis.py)
...
offset from the **mix audio itself** (independent of what the labeler dialed). BB11 + BB12: 521
clips → 302 (set, slot, stem) units, **296 reliable** (cross-corr peak ≥ 0.30, not window-saturated).
...
mix of wrong rips would be sign-balanced). Re-measuring on the **vocal band (200–3500 Hz)** collapses
every one to ~0¢ with *higher* correlation (Drake – Forever −188¢ → +0.3¢ corr .67→.92; Roy Orbison
−236¢ → −0.3¢; Calvin Harris – My Way −294¢ → −0.0¢ corr .31→.85). The vocals were in tune; the
full-band cross-correlation was locking onto the bass-heavy bed under a buried vocal. Acappellas are
now measured vocal-band; the genuine offsets (Tove Lo +40¢, Ocho Cinco +22¢) survive unchanged.
