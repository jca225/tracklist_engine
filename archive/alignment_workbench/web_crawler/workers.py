...
from archive.alignment_workbench.web_crawler.captcha_solver import EmailCaptchaSolver, solve_captcha_api
from archive.alignment_workbench.web_crawler.config import AppConfig
from archive.alignment_workbench.web_crawler.data_models import DJSet, DJSetCrawl, ScrapeFailure
from archive.alignment_workbench.web_crawler.database import MusicDatabase
from archive.alignment_workbench.web_crawler.scraper import ScrapedSetData, scrape_dj_set
...
def run_serial(
    ...
    with sync_playwright() as p:
        ...
        try:
            for job in jobs:
                tracklist_id = job.get("tracklist_id")
                url = job.get("url")
                ...
                try:
                    ...
                    djset = DJSet(
                        ...
                        date_played=job.get("date") or "",
                        artists=job.get("artists"),
                        creator_name=job.get("creator_name"),
                        creator_url=job.get("creator_url"),
                        views=job.get("views"),
                        ided_tracks=job.get("ided_tracks"),
                        total_tracks=job.get("total_tracks"),
                        likes=job.get("likes"),
                        play_time=job.get("play_time"),
                        styles=job.get("styles"),
                    )

                    db.insert_set(djset)
                    db.insert_crawl(crawl)
                    db.insert_set_media_links(data.set_media_links)
                    db.insert_track_media_links(data.track_media_links)
                    db.insert_failures(data.failures)
                    db.insert_rows(data.rows)
