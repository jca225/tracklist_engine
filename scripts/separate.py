...
def _run_uvr(
    ...
    *,
    ...
    print(
        f">>> uvr chain: {[s.name for s in cfg.stages]} (device={device})", flush=True
    )
    load_r = uvr_chain_adapter.load(cfg, device=device)
    if not load_r.is_ok():
        print(
            f"ERROR uvr load: {load_r.error.kind} — {load_r.error.detail}",
            file=sys.stderr,
        )
        return 1
    root = out_dir / "uvr"
    ...
    sep_r = uvr_chain_adapter.separate(
        ...
        root,
        track_audio_id=0,
        byproducts_dir=bp_dir,
    )
    if not sep_r.is_ok():
        print(
            f"ERROR uvr separate: {sep_r.error.kind} — {sep_r.error.detail}",
            file=sys.stderr,
        )
        return 1
    _flatten(root)
...
def _run_roformer(
    ...
    *,
    ...
    cfg = (
        RoformerChainConfig.from_yaml(config)
        if config
        else RoformerChainConfig.default()
    )
    cfg = cfg.with_batch_size(batch_size)
    print(
        f">>> roformer {cfg.version} (device={device}, batch_size={batch_size})",
        flush=True,
    )
    load_r = roformer_chain_adapter.load(cfg, device=device)
    if not load_r.is_ok():
        print(
            f"ERROR roformer load: {load_r.error.kind} — {load_r.error.detail}",
            file=sys.stderr,
        )
        return 1
    root = out_dir / "roformer"
    t0 = time.time()
    sep_r = roformer_chain_adapter.separate(
        load_r.value, input_path, root, track_audio_id=0
    )
    if not sep_r.is_ok():
        print(
            f"ERROR roformer separate: {sep_r.error.kind} — {sep_r.error.detail}",
            file=sys.stderr,
        )
        return 1
    _flatten(root)
...
def _run_demucs(input_path: Path, out_dir: Path, *, device: str) -> int:
    ...
    if not load_r.is_ok():
        print(
            f"ERROR demucs load: {load_r.error.kind} — {load_r.error.detail}",
            file=sys.stderr,
        )
        return 1
    root = out_dir / "demucs"
    ...
    if not sep_r.is_ok():
        print(
            f"ERROR demucs separate: {sep_r.error.kind} — {sep_r.error.detail}",
            file=sys.stderr,
        )
        return 1
    _flatten(root)
...
def main() -> int:
    ...
    ap.add_argument(
        "--separator", choices=["uvr", "demucs", "roformer", "both"], default="uvr"
    )
    ap.add_argument(
        "--config",
        type=Path,
    ...
    ap.add_argument(
        ...
        type=int,
        default=1,
        help="(roformer) chunks per GPU forward pass; 1=legacy. A-B 1 vs 8 to "
        "confirm identical output + measure speedup.",
    )
    args = ap.parse_args()
