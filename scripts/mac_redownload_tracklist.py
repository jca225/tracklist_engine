...
"""
    ...
    for line in _ssh_sql(sql).splitlines():
        ...
        if len(parts) < 7:
            continue
        tid, full_name, artists_json, title, version, ref_taid, ref_plat = parts[:7]
        try:
...
def _pick_video(c: Candidate) -> str | None:
    ...
    if isinstance(sr, Err):
        ...
        return None
    for hit in sr.value:
        if (hit.duration_s or 0) > 1200:
            continue
        _log.info("%s pick %s (%ss): %s", c.track_id, hit.video_id, hit.duration_s, hit.title)
        return hit.video_id
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    _log.info("loaded %d candidates (%d add / %d replace)",
              len(candidates), n_add, len(candidates) - n_add)

    ok = fail = 0
    t0 = time.monotonic()
    for i, c in enumerate(candidates, 1):
        _log.info("[%d/%d] %s mode=%s v=%s q=%r", i, len(candidates),
                  c.track_id, c.mode, c.version, c.query)
        vid = _pick_video(c)
        if not vid:
            fail += 1
            continue
