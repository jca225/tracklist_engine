...
def next_task(
    ...
    sql = (
        "SELECT ta.track_audio_id, COALESCE(ta.sha256, ''), ta.path "
        "FROM track_audio ta "
...
def fetch_measure_times(track_audio_id: int) -> tuple[float, ...]:
    ...
    if not out.strip():
        ...
        # quarantines this one track and continues.
        raise ValueError(
            f"no measure_times_json for track_audio_id={track_audio_id} "
            "(NULL beat grid or missing track_analysis row)"
        )
...
def main() -> int:
    ...
    # Scratch DB has no track_audio parent rows; disable FK enforcement so the
    # raw INSERTs of MERT rows don't trip on missing parents. Set here (not at
    # import) — see the note above the imports.
    ...
    while True:
        ...
        if nxt is None:
            ...
            log.log(level, "%s", msg)
            return code
        tid, expected_sha, remote_path = nxt
        ...
        try:
            ...
            # track. Verify the pull against canonical sha256 before embedding.
            # A mismatch is skipped for this run (re-pulled by a later process,
            # matching how the loop handles other pull failures).
            if expected_sha:
                try:
            ...
            # track, never crash the whole loop.
            log.error("[%d] failed: %s", tid, e)
            n_failed += 1
            failed_tids.add(tid)
        finally:
