...
def _download(vid: str, out: Path) -> bool:
    ...
    # B1: bound a yt-dlp bot-detection / broken-pipe stall.
...
def _push_and_register(job: Job, local: Path, video_id: str, *, dry_run: bool) -> int:
    ...
    scp = subprocess.run(
        ["scp", *SSH_OPTS, str(local), f"{PI_HOST}:{remote_tmp}"],
        capture_output=True,
        text=True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    jobs: tuple[Job, ...] = (
        ...
        Job(
            ...
            "GTA, Astronomar",
            "Heavy Thunder",
            frozenset({"naNj8bXnySE", "A7nR703Komg"}),
            "1652uk1",
            "re-acquire: pass1 removed 4500s playlist (naNj8bXnySE)",
            force_video_id=None,
        ),  # SKIP below — no YT hit; SC-only scrape
        Job(
            "1mlz2hg5",
            None,
            "KURA - Thunder",
            "KURA",
            "Thunder",
            frozenset({"naNj8bXnySE", "h5gNbrtlKQo"}),
            "2h9wg22t",
            "re-acquire: pass1 removed 4500s playlist (naNj8bXnySE)",
        ...
        Job(
            ...
            "Boundaries",
            frozenset({"aBq83ksKmUo", "LoRzim8svTA"}),
            "2uhd818t",
    ...
    for job in jobs:
        ...
        if job.track_id == "1fw35fxp" and job.force_video_id is None:
            print(
                "  SKIP here: SC-only — use ingest.main or replace_track_audio --url "
