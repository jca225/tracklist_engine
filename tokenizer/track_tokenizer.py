...
class TrackRow:
    ...
    # artwork
    artwork_url: Optional[str] = None             # cover art (data-src on img.artwork)
...
def parse_track_row(row_html: str) -> TrackRow:
    ...
    if art:
        candidate = art.get("data-src") or art.get("src") or ""
        # Reject the empty.png / transparent placeholder images.
        if candidate and "empty.png" not in candidate and "blank" not in candidate:
            tr.artwork_url = candidate
